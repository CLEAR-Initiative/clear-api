...
// ──────────────────────────────────────────────────────────────────────────
// Signal.rawData field resolver — DB-free (pure role check, no prisma call).
// Not inside describeIfDb above: this doesn't touch the database at all, so
// it should run everywhere, DB or not.
// ──────────────────────────────────────────────────────────────────────────
describe("Signal.rawData field resolver", () => {
  const rawPayload = { id: 42, figure: 1234, createdBy: "user-internal-id" };

  function ctxWithRole(role: string | undefined): Context {
    return {
      user: role ? ({ role } as Context["user"]) : null,
      prisma: undefined as unknown as Context["prisma"],
      session: null,
  ...
  it("returns the payload for an admin", () => {
    const result = signalResolvers.Signal.rawData(
      { rawData: rawPayload },
      {},
      ctxWithRole("admin"),
    );
  ...
  it("returns the payload for a pipeline-role caller", () => {
    const result = signalResolvers.Signal.rawData(
      { rawData: rawPayload },
      {},
      ctxWithRole("pipeline"),
    );
    expect(result).toEqual(rawPayload);
  });

  it("returns null for a viewer, never the raw payload", () => {
    const result = signalResolvers.Signal.rawData(
      { rawData: rawPayload },
      {},
      ctxWithRole("viewer"),
  ...
  it("returns null for an unauthenticated caller", () => {
    const result = signalResolvers.Signal.rawData(
      { rawData: rawPayload },
      {},
      ctxWithRole(undefined),
    );
    expect(result).toBeNull();
  });

  it("returns null (not undefined/throw) when rawData is absent, even for admin", () => {
    const result = signalResolvers.Signal.rawData({ rawData: undefined }, {}, ctxWithRole("admin"));
    expect(result).toBeNull();
  });
});
